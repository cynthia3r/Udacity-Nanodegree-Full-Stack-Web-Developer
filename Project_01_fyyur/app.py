...
def index():
  ...
  recent_listed_artists = Artist.query.order_by(Artist.id.desc()).limit(10).all()
  recent_listed_venues = Venue.query.order_by(Venue.id.desc()).limit(10).all()
  return render_template('pages/home.html', recent_listed_artists=recent_listed_artists, recent_listed_venues=recent_listed_venues)
...
def venues():
  ...
  data = []
  venues_in_state_city = db.session.query(Venue.city, Venue.state).group_by(Venue.city,Venue.state)

  for venue_regions in venues_in_state_city:
    venue_records = db.session.query(Venue).filter_by(city=venue_regions[0]).filter_by(state=venue_regions[1]).all()
    venue_result = []
...
def show_venue(venue_id):
  ...
  past_shows_result = db.session.query(Show).filter(Show.c.venue_id == venue.id).filter(Show.c.start_time <= datetime.now()).all()
  past_shows = []
  upcoming_shows_result = db.session.query(Show).filter(Show.c.venue_id == venue.id).filter(Show.c.start_time > datetime.now()).all()
  upcoming_shows = []

  for show in past_shows_result:
    artist_name = db.session.query(Artist.name).filter(Artist.id == show.artist_id)[0][0]
    artist_image_link = db.session.query(Artist.image_link).filter(Artist.id == show.artist_id)[0][0]
    past_shows.append({
...
def show_artist(artist_id):
  ...
  artist = Artist.query.get(artist_id)
  past_shows_result = db.session.query(Show).filter(Show.c.artist_id == artist.id).filter(Show.c.start_time <= datetime.now()).all()
  past_shows = []
  upcoming_shows_result = db.session.query(Show).filter(Show.c.artist_id == artist.id).filter(Show.c.start_time > datetime.now()).all()
  upcoming_shows = []

  for show in past_shows_result:
    venue_name = db.session.query(Venue.name).filter(Venue.id == show.venue_id)[0][0]
    venue_image_link = db.session.query(Venue.image_link).filter(Venue.id == show.venue_id)[0][0]
    past_shows.append({
...
def create_show_submission():
  ...
  try:
    ...
    show = Show.insert().values(venue_id=venue_id, artist_id=artist_id, start_time=start_time)
